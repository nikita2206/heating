...
public:
    explicit Impl(const ManagerConfig& config)
        : config_(config)
        , manualWriteSem_(xSemaphoreCreateBinary())
    {
    }
    ...
    ~Impl() {
        stop();
        if (manualWriteSem_) {
            vSemaphoreDelete(manualWriteSem_);
        }
    }

    esp_err_t start(Queues* queues) {
        if (!queues || !queues->isValid()) {
    ...
    ManagerStatus status() const {
        refreshControlState();
        ManagerStatus s;
        s.controlEnabled = controlEnabled_;
        s.controlActive = controlActive_;
        s.fallbackActive = fallbackActive_;
    ...
    esp_err_t writeData(uint8_t dataId, uint16_t dataValue,
        ...
        if (manualWritePending_) {
            return ESP_ERR_INVALID_STATE;
        }

        Frame request = Frame::buildRequest(MessageType::WriteData, dataId, dataValue);

        xSemaphoreTake(manualWriteSem_, 0);
        manualWriteFrame_ = request;
        manualWritePending_ = true;
        manualWriteResult_ = ESP_FAIL;

        if (xSemaphoreTake(manualWriteSem_, pdMS_TO_TICKS(timeout.count())) == pdTRUE) {
            response = manualWriteResponse_;
            return manualWriteResult_;
        }
...
private:
    ...
    std::optional<Frame> handleControlMode(uint8_t dataId) {
        switch (dataId) {
            case DataId::Status: {
                uint16_t status = buildStatusWord(demandChEnabled_);
                return Frame::buildResponse(MessageType::ReadAck, dataId, status);
            }
            case DataId::TSet: {
                float tset = demandTsetC_ > 0 ? demandTsetC_ : diagnostics_.tSetpoint.valueOr(0);
                return Frame::buildResponse(MessageType::ReadAck, dataId, floatToF88(tset));
            }
            case DataId::SlaveConfig:
            case DataId::RelModLevel:
                return Frame::buildResponse(MessageType::ReadAck, dataId, 0);
            default:
                return std::nullopt;
        }
    }

    void refreshControlState() const {
        // Get MQTT state - note: using global singleton pattern matching C version
        // In a real refactor, MqttBridge would be injected
        MqttConfig cfg;
        (void)MqttBridge::loadConfig(cfg);

        // For now, we'll leave MQTT state refresh as a TODO
        // The C++ MqttBridge would need to expose a global state accessor
        // or be injected into BoilerManager
    }
    ...
    size_t diagIndex_ = 0;
    int64_t lastDiagPollMs_ = 0;

    // Control state
    bool controlEnabled_ = false;
    mutable bool controlActive_ = false;
    mutable bool fallbackActive_ = false;
    mutable float demandTsetC_ = 0.0f;
    mutable bool demandChEnabled_ = false;
    mutable std::chrono::milliseconds lastDemandTime_{0};

    // Loop state
    LoopState loopState_ = LoopState::Idle;
    Frame pendingRequest_;
    bool pendingPassthrough_ = false;
    uint32_t id0FrameCounter_ = 0;

    // Manual write
    std::atomic<bool> manualWritePending_{false};
    Frame manualWriteFrame_;
    Frame manualWriteResponse_;
    esp_err_t manualWriteResult_ = ESP_FAIL;
    SemaphoreHandle_t manualWriteSem_;
...
esp_err_t BoilerManager::start(Queues* queues) {
    return impl_->start(queues);
}
