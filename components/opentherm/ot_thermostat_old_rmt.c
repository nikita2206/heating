                                            ...
                                            uint32_t *frame)
{
    ...
    for (int i = 0; i < sym_count && hb_count < 156; i++) {
        ...
        for (int part = 0; part < 2 && hb_count < 156; part++) {
            ...
            if (dur > 1500) {
                ESP_LOGD(TAG, "Skipping long duration: %d µs", dur);
                continue;
            }
    ...
    // Workaround: If we're exactly 1 half-bit short, and last symbol ended with level1=1 (HIGH),
    // assume the missing half-bit is the stop bit's HIGH period that wasn't captured
    if (hb_count == 67 && sym_count > 0) {
        const rmt_symbol_word_t *last = &syms[sym_count - 1];
        if (last->level1 == 1 && last->duration1 == 0) {
            // Last symbol transitioned to HIGH but didn't capture duration
            // This is the stop bit's HIGH period - add it
            half_bits[hb_count++] = 1;
            ESP_LOGI(TAG, "Applied workaround: padded missing stop bit HIGH half-bit");
        }
    }

    // Log extracted half-bits
    ESP_LOGI(TAG, "Extracted %d half-bits from %d symbols (need 68+)", hb_count, sym_count);

    // Print half-bits in groups of 10 for readability
    for (int i = 0; i < hb_count; i += 10) {
        char buf[64];
        int pos = 0;
        pos += snprintf(buf + pos, sizeof(buf) - pos, "HB[%2d-%2d]: ", i, (i + 9 < hb_count) ? i + 9 : hb_count - 1);
        for (int j = 0; j < 10 && (i + j) < hb_count; j++) {
            pos += snprintf(buf + pos, sizeof(buf) - pos, "%d", half_bits[i + j]);
        }
        ESP_LOGI(TAG, "%s", buf);
    }

    // Find start bit: scan for first '01' pattern (bit '1' = LOW-HIGH)
    // Need 68 half-bits total: start(2) + data(64) + stop(2)
    // So if start is at position i, we need i+67 to be valid, i.e., i <= hb_count-68
    int start_pos = -1;
    for (int i = 0; i <= hb_count - 68 && i + 1 < hb_count; i++) {
        if (half_bits[i] == 0 && half_bits[i + 1] == 1) {
            start_pos = i;
            ESP_LOGI(TAG, "Found start bit at position %d", i);
            break;
        }
    }

    if (start_pos < 0) {
        ESP_LOGI(TAG, "No start bit found in %d half-bits", hb_count);
    ...
    for (int i = start_pos + 2; i + 1 < hb_count && bit_count < 32; i += 2) {
        ...
        if (first == 0 && second == 1) {
            ...
            ESP_LOGW(TAG, "Invalid Manchester at HB %d-%d: %d%d", i, i+1, first, second);
            invalid_count++;
            continue;
        }
    }

    ESP_LOGI(TAG, "Decoded %d bits, %d invalid transitions", bit_count, invalid_count);

    if (bit_count != 32) {
        ESP_LOGI(TAG, "Only decoded %d bits (need 32)", bit_count);
        return ESP_ERR_INVALID_RESPONSE;
    }

    ESP_LOGI(TAG, "Decoded frame: 0x%08lX", (unsigned long)decoded);
    ...
    if (ones % 2 != 0) {
        ESP_LOGI(TAG, "Parity error (odd number of 1s: %d)", ones);
        return ESP_ERR_INVALID_CRC;
    }

    ESP_LOGI(TAG, "Parity OK");
    *frame = decoded;
    return ESP_OK;
}
 ...
 * Start RMT receive.
 *
 * Uses 1200µs idle timeout - just long enough to capture stop bit + minimal idle.
 * Each OpenTherm bit is 1000µs, so 1200µs allows for the stop bit plus margin.
 */
static esp_err_t start_rx(ot_thermostat_t *ctx)
{
    ...
    rmt_receive_config_t rx_config = {
        ...
        .signal_range_max_ns = 1200000,     // 1.2ms per-pulse timeout
    };
                                           ...
                                           uint32_t timeout_ms)
{
    ...
    while ((xTaskGetTickCount() - start_time) < timeout_ticks) {
        ...
        // Log all symbols for detailed debugging
        ESP_LOGI(TAG, "=== RX %d symbols ===", sym_count);
        for (int i = 0; i < sym_count; i++) {
            ESP_LOGI(TAG, "  [%2d] L0=%d D0=%4u  L1=%d D1=%4u",
                     i,
                     iface->rx_buffer[i].level0, iface->rx_buffer[i].duration0,
...
ot_thermostat_t* ot_thermostat_init(const ot_thermostat_config_t *config)
{
    ...
    // ESP32 classic doesn't support DMA for RX, using maximum mem_block_symbols instead
    rmt_rx_channel_config_t rx_config = {
        .gpio_num = config->rx_pin,
        .clk_src = RMT_CLK_SRC_DEFAULT,
        .resolution_hz = 1000000,
        .mem_block_symbols = 64,      // Max for ESP32 without DMA
        .flags.invert_in = true,      // Hardware inverts signal
        .flags.with_dma = false,      // Not supported on ESP32 classic
    ...
    // Allocate RX buffer (larger to accommodate full frame + idle)
    ctx->iface.rx_buffer = calloc(256, sizeof(rmt_symbol_word_t));
    ctx->iface.rx_buffer_size = 256;
