 ...
 * RMT operations for the thermostat interface.
 *
 * Thread loop:
 * 1. BLOCKS waiting for request from thermostat (up to 1100ms)
                                              ...
                                              void *user_ctx)
{
    ...
    thermostat_rx_ctx_t *ctx = (thermostat_rx_ctx_t *)user_ctx;
 ...
 * OpenTherm frame from the thermostat.
 *
 * @param ctx Thermostat context
 ...
 */
static esp_err_t thermostat_receive_frame(ot_thermostat_t *ctx, uint32_t *frame, uint32_t timeout_ms)
{
    ...
    // Decode the frame using the RMT library's decoder
    // (We'll inline a simple decoder here to avoid circular dependencies)
    if (symbol_count < 1) {
...
/**
 * Thermostat thread main loop
 *
 * This task runs continuously, handling all communication with the thermostat.
 * Each iteration:
 * 1. Receives a request from the thermostat (BLOCKS)
 * 2. Puts the request on the queue for the main loop
 * 3. Waits for a response from the main loop (BLOCKS)
 * 4. Sends the response to the thermostat (BLOCKS)
 */
static void ot_thermostat_task(void *arg)
{
    ...
    while (ctx->running) {
        ...
        if (ret == ESP_OK) {
            ESP_LOGD(TAG, "Received request from thermostat: 0x%08lX", (unsigned long)request_frame);

            // 2. Put request on queue for main loop (non-blocking, overwrites if full)
            msg.data = request_frame;
            xQueueOverwrite(ctx->queues->thermostat_request, &msg);

            // 3. BLOCKING: Wait for response from main loop (up to 750ms per OT spec)
            if (xQueueReceive(ctx->queues->thermostat_response, &msg, pdMS_TO_TICKS(750)) == pdTRUE) {
                response_frame = msg.data;
                ESP_LOGD(TAG, "Sending response to thermostat: 0x%08lX", (unsigned long)response_frame);
                ...
                // No response from main loop in time - thermostat will retry
                ESP_LOGD(TAG, "No response from main loop, thermostat will retry");
            }
        } else if (ret != ESP_ERR_TIMEOUT) {
            // Error other than timeout
            ESP_LOGD(TAG, "Receive error: %s", esp_err_to_name(ret));
        }
    }
