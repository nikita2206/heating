...
static esp_err_t boiler_receive_frame(ot_boiler_t *ctx, uint32_t *frame, uint32_t timeout_ms)
{
    ...
    // Workaround for ESP32 RMT hardware limitation: If exactly 67 half-bits and last
    // symbol ended with level1=1 duration1=0, pad missing stop bit HIGH half-bit
    if (hb_count == 67 && symbol_count > 0) {
        const rmt_symbol_word_t *last = &iface->rx_buffer[symbol_count - 1];
        if (last->level1 == 1 && last->duration1 == 0) {
            half_bits[hb_count++] = 1;
            ESP_LOGD(TAG, "Applied RMT workaround: padded missing stop bit");
        }
    }
